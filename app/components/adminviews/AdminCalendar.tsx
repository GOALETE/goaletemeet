...
export default function AdminCalendar() {
  ...
  const [syncResults, setSyncResults] = useState<{ created: number; updated: number; deleted: number } | null>(null);
  ...
  const handleConfirmSync = async () => {
    ...
    setSyncStatus(null);
    setSyncProgress(0);
    setSyncStep(2); // Move to progress view
    setSyncResults(null);

    // Simulate progress stages
    const progressStages = [
      { stage: 'Connecting to Google Calendar...', progress: 20 },
      { stage: 'Searching for GOALETE events...', progress: 40 },
      { stage: 'Processing calendar data...', progress: 60 },
      { stage: 'Updating database...', progress: 80 },
      { stage: 'Finalizing sync...', progress: 95 }
    ];
    ...
    try {
      ...
      for (let i = 0; i < progressStages.length; i++) {
        setSyncStage(progressStages[i].stage);
        setSyncProgress(progressStages[i].progress);
        await new Promise(resolve => setTimeout(resolve, 800)); // Wait 800ms between stages
      }
                ...
                <div className="space-y-4">
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-blue-800 font-medium">Calendar Sync in Progress</span>
                    </div>
                    ...
                    {/* Results Display (when sync is complete) */}
                    {syncResults && syncProgress === 100 && (
                      <div className="mt-4 pt-3 border-t border-blue-200">
                        <div className="grid grid-cols-3 gap-3 text-center">
                          <div className="bg-green-50 border border-green-200 rounded-lg p-2">
                            <div className="text-lg font-bold text-green-700">{syncResults.created}</div>
                            <div className="text-xs text-green-600">Created</div>
                          </div>
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
                            <div className="text-lg font-bold text-blue-700">{syncResults.updated}</div>
                            <div className="text-xs text-blue-600">Updated</div>
                          </div>
                          <div className="bg-orange-50 border border-orange-200 rounded-lg p-2">
                            <div className="text-lg font-bold text-orange-700">{syncResults.deleted}</div>
                            <div className="text-xs text-orange-600">Deleted</div>
                          </div>
                        </div>
                      </div>
                    )}
